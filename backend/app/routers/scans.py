...
def export_scan_pdf(
    ...
    class EvidencePDF(FPDF):
        ...
        def footer(self) -> None:
            ...
            self.cell(0, 8, f"Generated by VigiliCloud  |  {scan_date}  |  CIS AWS Foundations Benchmark v1.4  |  Scan {scan_id[:8]}", align="C")
    ...
    for i, row in enumerate(sorted_fail):
        ...
        pdf.cell(col_w[1], 6.5, str(row.get("service", ""))[:16], border="B", fill=True)
        pdf.cell(col_w[2], 6.5, str(row.get("check_id", ""))[:40], border="B", fill=True)
    ...
    if pass_rows:
        ...
        for i, row in enumerate(pass_rows):
            ...
            pdf.set_font("Helvetica", "B", 7)
            pdf.cell(pass_cols[0], 6.5, str(row.get("service", ""))[:14], border="B", fill=True)
            pdf.set_text_color(*BODY_TEXT)
            pdf.set_font("Helvetica", "", 7)
            pdf.cell(pass_cols[1], 6.5, str(row.get("check_id", ""))[:42], border="B", fill=True)
            res = str(row.get("resource_id", ""))
